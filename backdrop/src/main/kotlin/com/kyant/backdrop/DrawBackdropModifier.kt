...
private class DrawBackdropElement(
    ...
    override fun equals(other: Any?): Boolean {
        ...
        if (sampleScale != other.sampleScale) return false
    ...
    override fun hashCode(): Int {
        ...
        result = 31 * result + sampleScale.hashCode()
        return result
    }
}
...
private class DrawBackdropNode(
    ...
    private val drawBackdropLayer: DrawScope.() -> Unit = {
        ...
        if (layer != null) {
            ...
            val recordWidth =
                if (s == 1f) size.width.toInt() else (size.width * s).toInt()
            val recordHeight =
                if (s == 1f) size.height.toInt() else (size.height * s).toInt()
            recordLayer(
                layer,
                size = IntSize(
                    recordWidth + padding.toInt() * 2,
                    recordHeight + padding.toInt() * 2
                ),
                block = {
                    if (s == 1f) recordBackdropBlock()
                    else withTransform({ scale(s, s) }) { recordBackdropBlock() }
                }
            )
            ...
            if (s == 1f) {
                ...
                withTransform({ scale(1f / s, 1f / s) }) { drawLayer(layer) }
            }
        }
    }
    ...
    override fun ContentDrawScope.draw() {
        if (effectScope.update(this)) {
            // sampleScale < 1：效果链(blur 半径/padding 的 px)按比例缩小 → 录到半分辨率层再放大，
            // 屏幕空间视觉不变，GPU 填充量省 sampleScale^2
            if (sampleScale != 1f) effectScope.density = effectScope.density * sampleScale
            updateEffects()
        }
