...
internal class ShadowNode(
    ...
    // 性能：投影内容只取决于 尺寸/形状/半径/偏移/颜色/透明度 —— 这些不变时跳过重录，
    // 滚动（仅位移）不再每帧重跑一次大半径 BlurMaskFilter（原实现在每次 draw 无条件 record）。
    private var recordKey: Array<Any?>? = null
    ...
    override fun ContentDrawScope.draw() {
        ...
        if (shadowLayer != null) {
            ...
            val key = arrayOfAny(size.width, size.height, radius, offsetX, offsetY, shadow.alpha, shadow.color, shadow.blendMode, layoutDirection, shapeProvider.shape)
            ...
            if (key.contentDeepEquals(recordKey)) {
...
private fun arrayOfAny(vararg items: Any?): Array<Any?> = arrayOf(*items)
